...
class MjCambrianTrainerConfig(HydraContainerConfig):
    ...
    fitness_penalty_fn: Optional[
        Callable[Concatenate["MjCambrianConfig", ...], float]
    ] = None
...
class MjCambrianTrainer:
    ...
    def train(self) -> float:
        ...
        # Calculate fitness
        fitness, base_fitness, penalty = self._calculate_fitness()
        get_logger().info(
            f"Final Fitness: {fitness} (base={base_fitness}, penalty={penalty})"
        )

        # Save the final fitness to a file
        with open(self._config.expdir / "train_fitness.txt", "w") as f:
            f.write(str(fitness))
        with open(self._config.expdir / "train_fitness_base.txt", "w") as f:
            f.write(str(base_fitness))
        with open(self._config.expdir / "train_fitness_penalty.txt", "w") as f:
            f.write(str(penalty))
    ...
    def eval(
        ...
        **callback_kwargs,
        ...
        with open(self._config.expdir / f"{filename}_fitness.txt", "w") as f:
            f.write(str(fitness))
        with open(self._config.expdir / f"{filename}_fitness_base.txt", "w") as f:
            f.write(str(base_fitness))
        with open(self._config.expdir / f"{filename}_fitness_penalty.txt", "w") as f:
            f.write(str(penalty))
    ...
    def _calculate_fitness(self) -> tuple[float, float, float]:
        base_fitness = self._config.trainer.fitness_fn(self._config)
        penalty = 0.0
        if self._config.trainer.fitness_penalty_fn is not None:
            penalty = self._config.trainer.fitness_penalty_fn(self._config)
        return base_fitness - penalty, base_fitness, penalty
